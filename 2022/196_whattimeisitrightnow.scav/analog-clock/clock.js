// src: https://github.com/udivankin/sunrise-sunset
...
function isLeapYear(normieDate) {
  const startLeapYear = new Date('2022-05-05T14:45:00-05:00');
  const afterLeapYear = new Date('2022-05:05T15:00:00-05:00');
  return startLeapYear <= normieDate && normieDate < afterLeapYear; 
}

function getScavenDateFromNormieDate(date) {
  ...
  const normieMs = elapsedMsSinceScav * (numMsInNormieYear / numMsInScavenYear);

  const scavenTimestamp = leapYearStart + normieMs;
  const scavenDate = new Date(scavenTimestamp);

  return scavenDate;

  // Note: Scav uses "2014" as a leap year. To display this, we'll be using
  //       2016 instead. The setDate method will consider this.
}
...
class Clock {
  constructor() {
    ...
    this.dateNode = frag.querySelector('#thedate');
  ...
  updateTime(newTime) {
    ...
    date = getScavenDateFromNormieDate(date);
    ...
    // Lat/long for Ida: 41.7880° N, 87.5957° W
    // Note: Sunrise and sunset methods seem to be reversed
    const sunrise = SunriseSunsetJS.getSunset(41.7880, 87.5957, date);
    const sunset = SunriseSunsetJS.getSunrise(41.7880, 87.5957, date);
    if (date < sunrise) {
      this.setDark(true);
    } else if (date < sunset) {
      this.setDark(false);
    ...
    // Sat May 07 2022 18:26:45 GMT-0500 (Central Daylight Time)
    // Sat May 07 2022 08:13:42 GMT-0500 (Central Daylight Time)
    // Sat May 07 2022 17:59:18 GMT-0500 (Central Daylight Time)
  }
